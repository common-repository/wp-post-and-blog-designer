...
$wpbd_post_large_image = wpbd_get_post_featured_image( $post->ID, 'large' ); ?>
	<div class="wpbd-medium-12 wpbd-cell wpbd-left-block">
...
<?php } else {
	$wpbd_post_medium_image = wpbd_get_post_featured_image( $post->ID);
?>
	<div class="wpbd-medium-6 wpbd-cell <?php if($grid_count <= 1) { echo 'flotRight'; } ?> <?php if($grid_count == 1) { echo 'clearboth'; } ?>" >
		<div class="wpbd-post-right-block wpbd-medium-12 wpbd-cell">
			<?php if(!empty($wpbd_post_medium_image)) { ?>
				<div class="wpbd-medium-3 wpbd-cell">
					<div class="wpbd-post-image-bg">
						<a href="<?php echo esc_url( $post_link ); ?>">
							<img src="<?php echo esc_url( $wpbd_post_medium_image ); ?>" alt="<?php the_title_attribute(); ?>" />
						</a>
					</div>
				</div>
				<?php } ?>
				<div class="<?php if(!empty($wpbd_post_medium_image)) { echo 'wpbd-medium-9 wpbd-cell'; } else { echo 'wpbd-medium-12 wpbd-cell'; } ?> ">
					...
					if($postContent == "true") { ?>
					<div class="wpbd-post-content">
						<div class="wpbd-post-short-content"><?php echo wpbd_get_post_excerpt( $post->ID, get_the_content(), $words_limit); ?></div>
						<?php if($postreadmore == 'true') { ?>
						<a href="<?php echo esc_url( $post_link ); ?>" class="wpbd-readmorebtn"><?php echo _e('Read More', 'blog-designer-pack'); ?></a>
						<?php } ?>
					</div>
