...
?>
    <div class="wpbd-post-slides">
        <div class="wpbd-post-image-bg wpbd-medium-7 wpbd-cell">
            <?php if( !empty($feat_image) ) { ?>
                <a href="<?php echo esc_url( $post_link ); ?>">
			<img src="<?php echo esc_url( $feat_image ); ?>" alt="<?php the_title_attribute(); ?>" />
		</a>
                <div class="wpbd-post-image-bg-title">
        ...
        <div class="wpbd-post-overlay wpbd-medium-5 wpbd-cell ">
            <div class="wpbd-slider-inner-content ">
                <?php if($postDate == "true" || $postAuthor == 'true' || $post_comments =="true") { ?>
                    <div class="wpbd-post-date">
						...
						<a href="<?php the_permalink(); ?>#comments"><i class="fa fa-comments" aria-hidden="true"></i><i class="fa fa-comments" aria-hidden="true"></i><?php echo $comments.' '.$reply;  ?></a>
					</span>
                                                    <?php } ?>
                    </div>
                    <?php }
		              if($postContent == "true") { ?>
                        <div class="wpbd-post-content">
                            <div class="wpbd-post-short-content"><p>
                                <?php echo wpbd_get_post_excerpt( $post->ID, get_the_content(), $words_limit); ?></p>
                            </div>
                            <?php if($postreadmore == 'true') { ?>
                             <a href="<?php echo esc_url( $post_link ); ?>" class="wpbd-readmorebtn">
                                    <?php echo _e('Read More', 'blog-designer-pack'); ?>
                             </a>
                                <?php } ?>
                        </div>
                        <?php }
		if(!empty($tags) && $post_tags == 'true') { ?>
                            <div class="wpbd-post-tags"><i class="fa fa-tags" aria-hidden="true"></i>
                                <?php echo $tags; ?>
                            </div>
                            <?php } ?>
            </div>
